...
if os.path.exists(path1):
    result_O = CrawlerFunction.ReadFromJson(path1)
    # 获取新增的数据
    result_Append = list(set(result).difference(set(result_O)))
    # 保存新增的数据
    CrawlerFunction.WriteToJson(path2,result_Append)

# 保存下载的数据
CrawlerFunction.WriteToJson(path1,result)
